...
(defn build-satisfied-rules-map [rules transposed-tickets]
  (set/map-invert (reduce (fn [acc [i rule]] (update acc i (fnil conj []) rule))
                          {}
                          (->> (get-satisfied-rules rules transposed-tickets)))))
```

Note I also call `(set/map-invert)`. This swaps the keys and the values in the map.

So now the example maps looks like this:

```clojure
{["row"] 0, ["class" "row"] 1, ["class" "row" "seat"] 2}
```

At this point we have all the possible fields for each ticket position.
...
(defn solution []
  (let [input (read-input)
        ...
        satisfied-rules (build-satisfied-rules-map rules transposed-tickets)]
