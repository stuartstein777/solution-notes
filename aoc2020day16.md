...
```
Now its just a case of using the function `not-any?` to check if their are any tickets which don't return true when checked if they are between the 2 values in that list of boundaries.
...
This is just checking that a single entry in a ticket is within all boundaries for all rules.
